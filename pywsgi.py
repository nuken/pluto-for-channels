...
def playlist(provider, country_code):
    ...
    m3u = "#EXTM3U\r\n\r\n"
    for s in stations:
        url = f"http://{host}/{provider}/{country_code}/watch/{s.get('id')}\n\n"
        ...
        m3u += f" tvc-guide-categories=\"{s.get('group', '')}\""
...
def epg_xml(provider, country_code, filename):
    ...
    try:
        ...
        if filename in ALLOWED_EPG_FILENAMES:
            return send_file(file_path, as_attachment=False, download_name=file_path, mimetype='text/xml')
